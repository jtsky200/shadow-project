...
import { troubleshootingService, type TroubleshootingCategory, type TroubleshootingIssue } from "@/lib/firebase-service"

// Use the types from firebase-service.ts
type Issue = TroubleshootingIssue;

// Define type for category with JSX element for icon
type Category = Omit<TroubleshootingCategory, 'icon'> & {
  icon: string;
      ...
      try {
        ...
        if (fetchedCategories.length) {
          ...
          if (!selectedCategory && categoriesWithIssues.length > 0) {
            setSelectedCategory(categoriesWithIssues[0].id);
          }
        }
      } catch (error) {
